...
class API_Object_Core {
    
    protected $mspapi;
    public $result;


    public function __construct(API_Client $mspapi) {
        $this->mspapi = $mspapi;
    }
    ...
    protected function performMspCall($http_method, $api_method, $http_body = NULL) {
        ...
        if (!($object = @json_decode($body))) {
            throw new API_Exception("'{$body}'.");
        }

        if (!empty($object->error_code)) {
            $exception = new API_Exception("{$object->error_code}: {$object->error_info}.");
            throw $exception;
        }
        return $object;
    }
